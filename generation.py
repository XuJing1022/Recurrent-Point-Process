...
class MarkedIntensityHomogenuosPoisson(Intensity):
    ...
    def getValue(self, t, inds):  # 取self.lam的前len(inds)个
        l = len(inds)
        inten = [0] * l
        for i in range(l):
            inten[i] = self.lam[i]
    ...
    def getUpperBound(self, from_t, to_t, inds):  # 取self.lam的前len(inds)个
        l = len(inds)
        inten = [0] * l
        for i in range(l):
            inten[i] = self.lam[i]
...
def generate_samples_marked(intensity, T, n):  # 生成标注样本
    ...
    Sequences = []
    inds = np.arange(U)
    for i in range(n):  # 生成n个sample
        seq = []
        t = 0  # 第i个时间戳
        while True:
            intens1 = intensity.getUpperBound(t, T, inds)
            # print(intens1)
            dt = np.random.exponential(1 / sum(intens1))  # 指数分布取随机数，sum(intens1)指均值lamda
            # print(dt)
            new_t = t + dt  # 下一时间戳
            # print(new_t)
            if new_t > T:
            ...
            # print(intens2)
            u = np.random.uniform()   # 均匀分布取随机数
            if sum(intens2) / sum(intens1) > u:
                # print(intens2)
                x_sum = sum(intens2)
                norm_i = [x / x_sum for x in intens2]
                # print(norm_i)
                dim = np.nonzero(np.random.multinomial(1, norm_i))  # multinomial: 多项分布; nonzero: 返回数组a中值不为零的元素的下标; dim取值在[0,len(norm_i)]
                seq.append([np.asscalar(dim[0]), new_t])  # np.asscalar: Convert an array of size 1 to its scalar equivalent.
            t = new_t
        if len(seq) > 1:
